...
from ppstructure.utility import parse_args, init_args
...
def xlsx2md(xlsx: BytesIO | Workbook):
    df = pd.read_excel(xlsx)
    df = df.fillna('')
    df.columns = [t if t != f'Unnamed: {i}' else '' for i,t in enumerate(df.columns.to_list())]
    df.columns = [t.replace(' ', '') for t in df.columns.to_list()]
    return df.to_markdown(index=False)


def load_model(
        args = None,
        det_model_dir: str = "inference/ch_PP-OCRv3_det_infer",
        rec_model_dir: str = "inference/ch_PP-OCRv3_rec_infer",
        table_model_dir: str = "inference/ch_ppstructure_mobile_v2.0_SLANet_infer",
        rec_char_dict_path: str = "../ppocr/utils/ppocr_keys_v1.txt",
        table_char_dict_path: str = "../ppocr/utils/dict/table_structure_dict_ch.txt",
    ):
    if args is None:
        parser = init_args()
...
def main(args, table_sys: TableSystem):
    img = cv2.imdecode(np.frombuffer(args.img, dtype=np.uint8), 1)
    pred_res, _ = table_sys(img)
    pred_html = pred_res["html"]
    pred_excel = BytesIO()
    from tablepyxl import tablepyxl
    wb = tablepyxl.document_to_workbook(pred_html)
    wb.save(pred_excel)
    return xlsx2md(pred_excel)
...
def infer(
    ...
    args = None,
    det_model_dir: str = "inference/ch_PP-OCRv4_det_server_infer",
    rec_model_dir: str = "inference/ch_PP-OCRv4_rec_server_infer",
    table_model_dir: str = "inference/ch_ppstructure_mobile_v2.0_SLANet_infer",
    rec_char_dict_path: str = "../ppocr/utils/ppocr_keys_v1.txt",
    table_char_dict_path:str = "../ppocr/utils/dict/table_structure_dict_ch.txt"
):
    if args is None:
        ...
        args.rec_char_dict_path = rec_char_dict_path
        args.table_char_dict_path = table_char_dict_path
    return main(args, table_sys)


if __name__ == "__main__":
    # args = parse_args()
    # table_sys = load_model(args)
    
    with open(r'../table_test.png', "rb") as f:
        img = f.read()
    table_sys = load_model()
    print(infer(img, table_sys))
    # print(main(args, table_sys))
